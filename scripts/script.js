(function($){
	...
	//afficher le form de commentaires au clic.
	$(".commentForm").hide();
	...
	$.fn.heplbox = function( settings ) {
		...
		var defaultSettings = {
				...
				cssPrefix:"heplbox-", //on donne une classe a la box, prefixee par 'heplbox-'
				overlayOpacity:0.66, //fenetre s'assombrit lorsque l'image s'affiche
				ZIndexBase: 9999 // Zindex, met element au dessus des autres
			};
		var elements = [],
			currentIndex = 0, //L'image sur laquelle on se trouve.
			settings = $.extend(defaultSettings,settings),
			$overlay,
			$container;
		

		var setup = function(){  //méthode pour vérifier images
			$overlay = $("body>div." + settings.cssPrefix + "overlay");
			$container = $("body>div." + settings.cssPrefix + "container");
			...
			if (!$container.size()){
				$container= $('<div class="' + settings.cssPrefix +'overlay"></div>').css({
					position: "absolute",
		...
		var elementClicked = function(e){
			e.preventDefault();
			currentIndex = elements.indexOf($(this).attr("href")); //indexOf retourne le numéro d'index de l'élément.
			openBox();
		};
			...
			var $img = $('<img src="'+ elements[currentIndex] +'" />').css({
				...
				cursor: "pointer"
					...
					$container.animate({
						...
						width: $img.width(),
						height: $img.height(),
						top: ($img.height()>$(window).height())?10:(($(window).height()-$img.height())/2),
						marginLeft: -($img.width()/2)
						},"fast",function(e){
							$img.remove().appendTo($container).fadeIn("fast").on("click.heplBox", nextImage);
						});
					});
		};
		...
		var nextImage = function(){
			if(elements.length === 1){
				...
				{
				currentIndex = currentIndex+1>=elements.length?0:currentIndex+1;
		...
		return this.each(function(){
			elements.push($(this).attr("href")); //ajoute url a l'interieur d'elements.
			$(this).on("click.helpbox", elementClicked);
